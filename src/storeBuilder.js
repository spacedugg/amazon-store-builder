  ...
  var system = [
    ...
    category && category !== 'generic' && CATEGORY_STYLE_HINTS[category]
      ? [
          ...
          CATEGORY_STYLE_HINTS[category].trustFocus ? 'Include trust elements and certifications.' : '',
        ].filter(Boolean).join('\n')
      : '',
    '',
    ...
    template ? [
      ...
      '╚══════════════════════════════════════════════════════════════╝',
      '',
      '=== TEMPLATE STYLE (high-level direction — designer handles details) ===',
    ...
    'Each brief MUST start with an IMAGE TYPE tag, then a SHORT image concept.',
    'The brief describes WHAT should be visible — NOT how to style it (no colors, shadows, textures).',
    'The designer knows the brand CI and will make style decisions independently.',
    '',
    'IMAGE TYPES:',
    '- [BENEFIT] = Narrow/compact image showing USPs, features, or benefits. Supportive, not dominant.',
    '  Example: "[BENEFIT] 3 key benefits of ' + brand + ' product with icons"',
    '',
    '- [CREATIVE] = Designed composition — product/lifestyle photo with text, graphics, brand elements.',
    '  Example: "[CREATIVE] ' + brand + ' category overview with product arrangement and category name"',
    '',
    '- [LIFESTYLE] = Product/people in real-world use. Scene and mood are dominant, text is secondary.',
    '  Example: "[LIFESTYLE] Person using ' + brand + ' product outdoors, product prominent"',
    '',
    '- [PRODUCT] = Clean product shot on simple background. Renderings or packshots.',
    '  Example: "[PRODUCT] ' + brand + ' flagship product, angled view"',
    '',
    '- [TEXTIMAGE] = Image where text is the primary content (Amazon stores have no native text styling).',
    '  Example: "[TEXTIMAGE] Brand statement headline with CTA button"',
    '',
    'BRIEF RULES:',
    '- Keep briefs SHORT: max 10-15 words after the tag.',
    '- Describe WHAT is visible (subject, scene, products). Do NOT describe HOW to style it.',
    '- Do NOT specify: colors, shadows, textures, backgrounds, lighting, typography style.',
    '- Name the specific ' + brand + ' product or category.',
    '- textOverlay MUST be in store language (' + lang + ').',
    '- Do NOT use generic placeholders like "[product]".',
    '',
    'MINIMUM SECTIONS:',
  ...
  var user = [
    ...
    isHomepage
      ? [
          ...
          '1. HERO BANNER (layout "1"): [LIFESTYLE] brand hero scene. textOverlay = slogan in ' + lang + '.',
          '2. CATEGORY NAVIGATION (layout based on count: 2="vh-2equal", 3="vh-w2s", 4="2x2wide", 5+="lg-4grid"):',
          '   Each tile = [CREATIVE] category scene with category name. textOverlay = EXACT category name.',
          '3. BESTSELLER SHOWCASE (layout "lg-2stack" or "2stack-lg"): [PRODUCT] hero product + [CREATIVE] feature tiles. Use real ASIN as linkAsin.',
          '4. PRODUCT GRID (layout "1"): type "product_grid" with top 5-8 products by rating.',
          '5. LIFESTYLE SPLIT (layout "std-2equal"): [LIFESTYLE] product in use + [PRODUCT] product with linkAsin.',
          '6. USP / FEATURE BAR (layout "vh-w2s" or "vh-2sw"): [BENEFIT] wide feature overview + two square benefit tiles. MUST use VH layout — not standard.',
          '7. BRAND STORY (layout "lg-2stack" or "2stack-lg"): [LIFESTYLE] large brand image + [TEXTIMAGE] brand values.',
          '8. FOOTER NAV (layout "2x2wide"): [CREATIVE] category tiles for bottom navigation.',
          '',
          'IMPORTANT: For USPs, benefits, features — ALWAYS use VH layouts (vh-w2s, vh-2sw, vh-2equal).',
          'VH layouts take less vertical space and are ideal for supportive content like benefits.',
          'NEVER use std-2equal for USP/benefit sections — it wastes too much space.',
      ...
      : [
          ...
          '2. PRODUCT HIGHLIGHT (layout "lg-2stack" or "2stack-lg"): [PRODUCT] bestseller with real ASIN + [CREATIVE] feature tile.',
          '3. USP / FEATURE BAR (layout "vh-w2s" or "vh-2sw"): [BENEFIT] key features. MUST use VH layout. Each textOverlay = feature name in ' + lang + '.',
          ...
          analysis.productComplexity === 'complex' || analysis.productComplexity === 'variantRich'
            ? '6. VARIANT SHOWCASE (layout "lg-4grid"): [PRODUCT] large hero + 4 variant tiles.'
            : '6. CROSS-SELL (layout "vh-w2s"): [CREATIVE] wide tile + two square tiles linking to related categories.',
          '',
          'IMPORTANT: For USPs, benefits, features — ALWAYS use VH layouts (vh-w2s, vh-2sw, vh-2equal). NEVER std-2equal.',
          'MINIMUM 5 sections. EVERY brief must start with [BENEFIT], [CREATIVE], [LIFESTYLE], [PRODUCT], or [TEXTIMAGE] tag + name specific ' + brand + ' products.',
        ].filter(Boolean).join('\n'),
  ].filter(Boolean).join('\n');
