import { uid, LAYOUTS, LAYOUT_TILE_DIMS, STORE_PRINCIPLES, MODULE_BAUKASTEN, PRODUCT_COMPLEXITY, CATEGORY_STYLE_HINTS, IMAGE_CATEGORIES, IMAGE_CATEGORY_DECISION_TREE, findLayout, resolveLayoutId, createDefaultProductSelector } from './constants';
...
// Homepage rule: product_grid never in the middle. Move it to the end so it
// acts as a conversion-finisher, never as a filler block between hero and content.
function enforceHomepageProductGridPlacement(homeSections) {
...
export async function aiGeneratePageLayout(pageName, pageProducts, brand, lang, isHomepage, allCategories, analysis, userInstructions, category, template, websiteData, referenceAnalysis, isSubpage, structuralBlueprint) {
  ...
  var system = [
    ...
    '- Adjacent sections should feel like they belong together, not like random independent blocks.',
    '- Two consecutive full-width images can form a visual PAIR that tells a combined story (e.g., lifestyle scene above, benefit summary below).',
    '- When describing briefs, reference relationships to neighboring sections:',
    ...
    '',
    'TEXT RULES:',
    '- textOverlay: Text designed INTO the image. In store language (' + lang + '). This is the ONLY field that uses the store language.',
    ...
    '- brief: ALWAYS in ENGLISH. This is a design instruction for the designer. Describe the image concept, scene, composition, and mood.',
    '  CRITICAL: Do NOT repeat textOverlay content in the brief. The textOverlay field already contains the exact text for the image.',
    '  Instead, describe the visual concept: what the image shows, the scene, mood, composition, people, setting.',
    '  WRONG brief: "Headline: Unsere Qualität, 3 USPs: BPA-frei, Made in Germany, Vegan"',
    '  RIGHT brief: "Split composition: quality seal left, product detail right, clean minimalist feel"',
    '- Native text: ONLY for section headings. NOT for marketing.',
    '- TEXT RULES for textOverlay and ctaText (customer-facing):',
    ...
    '    - A hyphen with a space before or after it (like " - ") is FORBIDDEN. Use comma, colon, line break (\\n), or a rewrite.',
    '- textOverlay supports multiple lines (use \\n). When a tile has MULTIPLE text elements, use \\n to separate them.',
    '  IMPORTANT: Prefix each line with a ROLE TAG so the designer knows how to style it:',
    '  [h1] = Heading, the main, most prominent text on the image',
    '  [h2] = Subheading, supporting text, smaller than heading',
    '  [body] = Body text, normal paragraph/descriptive text',
    '  [bullet] = Bullet point, equal-rank item in a list (all bullets have the same visual weight)',
    '  If ALL lines are equal-rank (e.g. 4 USPs, 4 category names), use [bullet] for all of them.',
    '  If one line is clearly a heading and the rest supports it, use [h1] + [h2] or [h1] + [body].',
    '',
    '  Examples:',
    '  - Heading + subheading: "[h1]Effektive Porenreinigung\\n[h2]Für ein klares Hautbild"',
    '  - 3 equal USPs: "[bullet]Made in Germany\\n[bullet]Laborgeprüft\\n[bullet]Vegan"',
    '  - Heading + bullets: "[h1]Unsere Qualität\\n[bullet]BPA-frei\\n[bullet]100% natürlich"',
    '  - Single text (no tag needed): "Vitamine"',
    '  If a tile has only ONE text element, do NOT add a tag — just write the text.',
    '',
    '  BULLET LIMIT FOR WIDE TILES:',
    '  In WIDE tiles (W 1500x750 or wider, spanning 2 columns): maximum 2 bullet points.',
    '  Wide tiles have limited vertical space — more than 2 bullets will be too cramped.',
    '  For LARGE SQUARE tiles (1500x1500): up to 4 bullets are OK.',
    '',
    '',
    ...
    template ? [
      ...
      '',
      'CRITICAL: Adapt EVERY brief to the brand — do NOT output generic/template text.',
      '- Replace ALL [product], [category product], generic references with ACTUAL product names from the product list.',
      '- Briefs should mention specific products by name or ASIN where relevant, but do NOT repeat the brand name in every single brief. The designer already knows the brand.',
      '- textOverlay MUST be in the store language and brand-specific (not "Product Name" but "K5 Premium" or the actual product name).',
    ...
    '- Use VARIED layouts. Full-width "1" layouts are VERY IMPORTANT for brand storytelling:',
    '  - Use full-width "1" for: lifestyle images, brand statements, text_image headings, category intros, mood images.',
    '  - A good page has 3-5 full-width sections mixed with 2-4 multi-tile grid sections.',
    '  - Full-width lifestyle/creative images create breathing room and emotional anchors between grid sections.',
    '- Mix grid layouts for product showcases: std-2equal, lg-2stack, lg-4grid, 2x2wide, vh-w2s, 2s-4grid.',
    '- ALTERNATE between full-width (1) and grid layouts. Pattern: Full-width → Grid → Full-width → Grid.',
    '- Sections flow: Hero → text_image heading → Product showcase grid → Lifestyle full-width → Feature grid → text_image heading → Product grid → Lifestyle full-width → Cross-sell.',
    '- Do NOT stack multiple grid sections without a full-width breathing section between them.',
    '',
    'TEXT_IMAGE SECTIONS — USE GENEROUSLY:',
    '- text_image tiles serve as section dividers, headings, and visual structure.',
    '- Use them to introduce new sections, categories, or topics.',
    '- They break up product-heavy sections and add design variety.',
    '- text_image tiles can feature: headlines, brand claims, feature callouts, category introductions.',
    '- The designer can add icons, graphics, patterns, and visual elements to text_image tiles.',
    '- Include at least 2-3 text_image sections per page as visual anchors between product/lifestyle content.',
    ...
    '  Example: "[STORE_HERO] Brand world, warm lifestyle backdrop, headline: Best Quality Since 2015"',
    '',
    ...
    '  Example: "[PRODUCT] Spray bottle, 45° angle, light gray bg, badge: NEW"',
    '',
    ...
    '  CREATIVE briefs should describe the visual CONCEPT and COMPOSITION:',
    '  - What elements are in the image (product, text zones, icons, graphics)',
    '  - How they are arranged (split layout, centered, layered, grid)',
    '  - What story the image tells (category overview, feature highlight, comparison)',
    '  GOOD: "[CREATIVE] Split composition: product hero shot left (45° angle), three ingredient icons stacked right with benefit labels"',
    '  GOOD: "[CREATIVE] Category overview: three product variants arranged in a row, category headline above, discover CTA below"',
    '',
    '- [LIFESTYLE] = Lifestyle photo dominates (70-80%+). Text is subordinate/optional.',
    '  Elements: professional photo, product in real-world use, optional short text line.',
    '  CRITICAL: Lifestyle briefs must be SPECIFIC and DESCRIPTIVE. Describe:',
    '  1. WHO: Person description (age range, gender, appearance hint)',
    '  2. WHERE: Setting (indoor/outdoor, specific environment like kitchen, gym, office, park)',
    '  3. WHAT: Activity (cooking, working out, relaxing, working, applying product)',
    '  4. HOW: Product interaction (holding, using, product visible on table/shelf)',
    '  5. MOOD: Emotional tone (satisfied, energetic, relaxed, focused)',
    '  Do NOT specify colors, overlays, gradients — the designer decides based on CI.',
    '  GOOD: "[LIFESTYLE] Woman mid-30s using face serum at vanity, morning light, close-up on application, product bottle on shelf, calm relaxed mood"',
    '  GOOD: "[LIFESTYLE] Father and daughter building toy together on living room floor, warm family atmosphere, product box visible, both smiling"',
    '  BAD: "[LIFESTYLE] Person using product" (too vague!)',
    '  BAD: "[LIFESTYLE] Product in lifestyle setting with warm tones" (describes style, not scene!)',
    '',
    ...
    '3. Lifestyle photo >70% with subtle text? → LIFESTYLE',
    '4. Combines 2-3 elements (product + text + graphics) equally? → CREATIVE',
    ...
    '',
    'BRIEF RULES (briefs are SHORT instructions for a DESIGNER):',
    '- Briefs MUST be 10-20 words MAX. Just the image idea and the key elements.',
    '- The designer works autonomously. Do NOT describe lighting, mood, camera angles, or atmosphere.',
    '- GOOD: "Woman drinking from bottle after jogging in park, product visible"',
    '- GOOD: "Split composition: product left, three ingredient icons right"',
    '- GOOD: "Brand founder portrait with product lineup, office setting"',
    '- BAD: "Cinematic wide shot: couple mid-30s preparing healthy breakfast in bright modern kitchen, warm morning sunlight, joyful interaction"',
    '- BAD: "Serene morning scene with soft natural light filtering through curtains"',
    '- The brief describes WHAT to show. The designer decides HOW it looks.',
    '- WRONG: "Specific background color with specific font style"',
    ...
    '- textOverlay MUST be in store language (' + lang + ') — use real product/category names. For category/navigation tiles: write an on-brand label (ideally a punchy benefit-led line OR the category name itself, whichever is stronger). Never settle for the raw category slug as a boring sub-headline — if nothing punchier fits, use the clean category name as the headline.',
    '',
    ...
    '- CROSS-SELL TO A SPECIFIC CATEGORY: Use imageCategory="creative" or "lifestyle" with a visual preview. textOverlay = a short on-brand label (category name OR a benefit-led headline that fits the brand voice). The CTA carries the action. Brief = SHORT, e.g. "[CREATIVE] [category] preview with representative product".',
    ...
    '',
    'SECTION COUNT — content-driven, not tier-driven:',
    '- Let the brand and the catalog decide how many sections a page needs. A clear brand with few products may need only 4 sections; a rich brand with 40+ products may justify 10.',
    '- Homepage: typical range 4-10 sections. Build more only if the brand voice and content inventory warrant it.',
    '- Category page: typical range 3-7 sections. Only add sections if they carry real content.',
    '- NEVER pad with filler. A short store that earns every section beats a long store that feels templated.',
    '',
    ...
    isHomepage
      ? (skipCategoryPages
        ? [
          ...
          'Generate 5-9 sections depending on how much the brand has to say. Every product appears AT MOST ONCE as shoppable_image, but can appear in lifestyle/creative context.',
          ...
          '- Briefs must reference specific products by name — generic briefs are FORBIDDEN. Do NOT repeat the brand name in every brief.',
        ].filter(Boolean).join('\n')
        : [
          '╔══════════════════════════════════════════════════════════════════╗',
          ...
          '   imageCategory="creative" for each tile. textOverlay = on-brand label for the category (punchy benefit-led headline OR the clean category name, whichever fits the brand voice — never a raw slug). CTA links to category page.',
          ...
          '',
          '4. BESTSELLER HIGHLIGHT (layout "std-2equal" or "lg-2stack") — OPTIONAL:',
          '   ONLY include if the brand has a clear bestseller product (highest reviews/rating).',
          ...
          '',
          '5. BRAND STORY (layout "lg-2stack" or "std-2equal") — OPTIONAL, only if the brand has real story material:',
          ...
          '   SKIP this section if there is no substantive brand story to tell.',
          '',
          'FINAL SECTION (OPTIONAL):',
          '6. LIFESTYLE / BRAND WORLD (layout "1"): imageCategory="lifestyle". One compelling brand-world image. Product in use, brand atmosphere. NO random product showcase. Include only if it adds emotional closure the homepage still needs.',
          '',
          ...
          '- A "New Arrivals" creative tile that links to the new products subpage',
          '- A "Product Finder" full-width image that links to a finder/overview subpage',
          '- Brand value tiles that link to an About Us subpage',
          '- A lifestyle image that links to a specific category',
          'Each clickable tile uses linkUrl="/cat-X" or linkUrl="/pageId" to link to the respective subpage.',
          '',
          ...
          analysis.productComplexity === 'complex' || analysis.productComplexity === 'variantRich'
            ...
            ? (function() {
              ...
              return [
                ...
                '   textOverlay = on-brand label for the category (benefit-led headline OR clean category name, whichever is stronger for the brand voice). Add a fitting CTA.',
...
// Content-driven minimum: enough sections so the page feels complete, scaled by catalog size.
function ensureMinimumSections(sections, pageName, brand, lang, analysis, template, isHomepage) {
  ...
  if (isSmallCatalogHomepage) {
    ...
    minSections = totalProducts <= 5 ? 3 : 4;
  } else {
    minSections = totalProducts <= 5 ? 2 : 3;
  }
...
export async function generateStore(asins, products, brand, marketplace, lang, userInstructions, onLog, template, websiteData, referenceAnalysis, featureOptions, cancelRef, pipelineData) {
  ...
  try {
    var homeResult = await aiGeneratePageLayout(
      ...
      analysis.categories || [], analysis, userInstructions, category, template, websiteData, referenceAnalysis, false
    );
    ...
    homeSections = enforceHomepageProductGridPlacement(homeSections);
  ...
  for (var ci = 0; ci < categories.length; ci++) {
    ...
    try {
      var catResult = await aiGeneratePageLayout(
        ...
        categories, analysis, userInstructions, category, template, websiteData, referenceAnalysis, false, categoryBlueprint
      );
      ...
      for (var si = 0; si < cat.subcategories.length; si++) {
        ...
        try {
          var subResult = await aiGeneratePageLayout(
            ...
            categories, analysis, userInstructions, category, template, websiteData, referenceAnalysis, true
          );
